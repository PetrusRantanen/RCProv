...
class EthernetSwitch
{
    ...
    protected $baseport2ifIndex;

    protected $ifIndex2name;
    ...
    public function __construct($ip, $community, $options = [])
    {
        $this->ip = $ip;
    ...
    public function getPortNameByIfIndex($ifIndex)
    {
        if (! $ifIndex) {
            return false;
        }
    ...
    /* Get ifIndex number by BRIDGE-MIB baseport number
       Results are cached, so multiple calls will not result in multiple SNMP queries */
    public function getIfIndexByBasePort($baseport)
    {
        if (! $baseport) {
            return false;
        }
    ...
    public function getMac2basePortTable()
    {
        $mac_addresses = [];
        $baseports = [];
        $result = [];
        ...
        foreach ($data as $oid => $v) {
            ...
            } elseif (! strncmp($oid, self::MACTABLE_PORT_OID, $l)) {
                $nr = substr($oid, $l + 1);
                $baseports[$nr] = $v;
            }
        }
    ...
    public function getMac2portNameTable()
    {
    ...
    public function getPortNameByMac($m)
    {
    ...
    public function _bin2mac($binary)
    {
        if (strlen($binary) != 6) {
