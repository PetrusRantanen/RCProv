...
clear
echo "==========="
echo "Starting to provision this device"
echo "==========="
echo ""
...
if [ -f /tmp/eeprom_version ]; then
    curl --retry 10 -g -F 'eeprom_version=@/tmp/eeprom_version' "http://{{ $server }}/scriptexecute?serial={{ $cm->serial }}" >> /dev/null
fi
@endif
...
echo "===" >> /tmp/pre.log
sh /tmp/pre-{{$script->id}}.sh >>/tmp/pre.log 2>&1 @if ($script->bg) & @endif 
RETCODE=$?

if [ $RETCODE -ne 0 ]; then
    echo "Pre-installation script failed."
    curl --retry 10 -g -F 'log=@/tmp/pre.log' "http://{{ $server }}/scriptexecute?serial={{ $cm->serial }}&retcode=$RETCODE&phase=preinstall" >> /dev/null
    exit 1 
fi
@endforeach

curl --retry 10 -g -F 'log=@/tmp/pre.log' "http://{{ $server }}/scriptexecute?serial={{ $cm->serial }}&retcode=0&phase=preinstall" >> /dev/null
@endif
...
if [ $RETCODE -eq 0 ]; then
    ...
    curl --retry 10 -g -F 'log=@/tmp/dd.log' "http://{{ $server }}/scriptexecute?serial={{ $cm->serial }}&retcode=$RETCODE&phase=dd" >> /dev/null
    exit 1
fi
...
if [ $RETCODE -ne 0 ]; then
    ...
    curl --retry 10 -g -F 'log=@/tmp/post.log' "http://{{ $server }}/scriptexecute?serial={{ $cm->serial }}&retcode=$RETCODE&phase=postinstall" >> /dev/null
    exit 1
fi
@endforeach
curl --retry 10 -g -F 'log=@/tmp/post.log' "http://{{ $server }}/scriptexecute?serial={{ $cm->serial }}&retcode=0&phase=postinstall" >> /dev/null
@endif

TEMP=`vcgencmd measure_temp`
curl --retry 10 -g "http://{{ $server }}/scriptexecute?serial={{ $cm->serial }}&alldone=1&temp=${TEMP:5}&verify={{ $project->verify }}" >> /dev/null

echo ""
echo "==="
echo "Provisioning completed successfully! You can turn off the power to the device"
